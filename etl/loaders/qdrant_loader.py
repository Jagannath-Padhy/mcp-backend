...
from qdrant_client.models import (
    ...
    CollectionStatus, CreateCollection,
    UpdateStatus, OptimizersConfig,
    HnswConfig, PayloadSchemaType
)
...
class QdrantLoader(BaseLoader):
    ...
    async def setup(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Could not retrieve server version info: {version_check_error}")
            ...
            try:
                logger.warning("Retrying connection with relaxed compatibility settings...")
                import warnings
                with warnings.catch_warnings():
                    ...
                    info = self.client.get_collections()
                    logger.info(f"Fallback connection successful to Qdrant at {self.host}:{self.port}")
    ...
    async def _create_collection_via_rest_api(self, 
                                            collection_name: str, 
                                            vector_size: int, 
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    logger.info(f"Successfully created collection '{collection_name}' via REST API")
                    return True
                    ...
                    if "already exists" in error_text.lower():
                        ...
                        if etl_mode == "upsert":
                            logger.info(f"Collection '{collection_name}' already exists, proceeding with upsert mode")
                            return True
                        else:
                            logger.warning(f"Collection '{collection_name}' already exists but mode is '{etl_mode}'")
                            return False
                    else:
                        logger.error(f"REST API collection creation failed: {response.status_code} - {response.text}")
                        return False
                else:
                    logger.error(f"REST API collection creation failed: {response.status_code} - {response.text}")
                    return False
    ...
    async def create_collection(self, 
        ...
        if await self.collection_exists(collection_name):
            if etl_mode == "recreate":
                logger.info(f"Collection '{collection_name}' exists. Mode is 'recreate', deleting existing collection...")
                ...
                logger.info(f"Successfully deleted collection '{collection_name}', will recreate...")
            elif etl_mode == "upsert":
                logger.info(f"Collection '{collection_name}' exists. Mode is 'upsert', will update existing data.")
        ...
        for attempt in range(max_retries):
            try:
                ...
                coll_config = config or self.collection_configs.get(
                    collection_name, 
                    self.collection_configs["products"]
                )
                ...
                try:
                    ...
                    logger.info(f"Created Qdrant collection with basic config: {collection_name}")
                    return True
                    
                except Exception as basic_error:
                    logger.warning(f"Basic collection creation failed: {basic_error}")
                    logger.info("Attempting collection creation with optimized settings...")
                    ...
                    try:
                        ...
                        logger.info(f"Created Qdrant collection with optimized config: {collection_name}")
                        return True
                        
                    except Exception as optimized_error:
                        logger.error(f"Optimized collection creation failed: {optimized_error}")
                        logger.warning("Attempting REST API fallback for collection creation...")
                        ...
                        if rest_success:
                            ...
                            raise Exception(f"REST API fallback also failed for collection '{collection_name}'")
                            
            except Exception as e:
                if attempt < max_retries - 1:
                    logger.warning(f"Collection creation attempt {attempt + 1} failed: {e}. Retrying in {retry_delay} seconds...")
                    await asyncio.sleep(retry_delay)
                    retry_delay *= 2  # Exponential backoff
                else:
                    logger.error(f"All {max_retries} collection creation attempts failed")
                    raise Exception(f"Failed to create collection '{collection_name}' after {max_retries} attempts: {e}")
    ...
    async def _create_payload_index(self, 
                                   collection_name: str,
        ...
        try:
            ...
            logger.info(f"Created payload index on {collection_name}.{field} ({field_type})")
    ...
    async def load_batch(self, 
        ...
        try:
            ...
            if not await self.collection_exists(collection_name):
                error_msg = f"Collection '{collection_name}' does not exist. Cannot load batch."
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if "doesn't exist" in str(upsert_error) or "404" in str(upsert_error):
                    ...
                    if attempt < max_retries - 1:
                        wait_time = 2 ** attempt
                        logger.warning(f"Upsert attempt {attempt + 1} failed: {upsert_error}. Retrying in {wait_time} seconds...")
            ...
            if operation_info.status == UpdateStatus.COMPLETED:
                ...
                logger.info(f"Successfully loaded {loaded_count} points to {collection_name}")
                ...
                return LoadResult(
                    ...
                    loaded_count=0,
                    failed_count=len(records),
                    errors=errors + [f"Qdrant operation failed: {operation_info.status}"],
                    metadata={"collection": collection_name},
                    loaded_at=datetime.utcnow(),
                    loader=self.loader_name
                )
    ...
    def _record_to_point(self, 
                        record: Dict[str, Any], 
                        index: int) -> Optional[PointStruct]:
        ...
        try:
            ...
            if not isinstance(embedding, list) or len(embedding) != self.vector_size:
                raise ValueError(f"Invalid embedding: expected list of {self.vector_size} floats")
            ...
            payload["id"] = original_id  # Also store as 'id' for consistency with search
    ...
    async def search_similar(self, 
        ...
        try:
            ...
            if filter_conditions:
                search_params["query_filter"] = self._build_qdrant_filter(filter_conditions)
    ...
    async def create_snapshot(self, collection_name: str) -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"Created snapshot {snapshot_name} for collection {collection_name}")
